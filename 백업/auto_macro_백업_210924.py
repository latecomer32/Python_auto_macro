...
# 키보드 타이핑 값 전역에 공유 (write_entry.bind("<Return>", send_to_type_write)->write_entry.bind("<Return>", update_type_write_motion_log)로 변경하고 update_type_write_motion_log(event)바디부분에 type_write_text=write_entry.get(); write_win_destroy()를 추가하는 식이면 아래 함수를 없애도 될거 같음.)
...
def update_left_click_motion_log():
    ...
    hidden_motion_log.insert(motion_num*2-1,"Left_click()")
...
def update_right_click_motion_log():
    ...
    hidden_motion_log.insert(motion_num*2-1,"Right_click()")
...
def save_coordinate_by_txt_file():
    ...
    print("mo:",hidden_motion_log.get(0,END))
    print("molen:",len(hidden_motion_log.get(0,END)))
    for inStr in inList:
        outFp.write(inStr)
    print("in:",type(inStr))
...
# 매크로 카탈로그 클릭 : 파일명 1개 이상 즉 다중 클릭시 다중 클릭된 목록 공유 
...
def selceted_list_delete_btn_click():
    print("selceted_list_delete_btn_click()")
...
def create_selceted_list_delete_btn():
    print("create_selceted_list_delete_btn()")
...
def delete_selceted_list_delete_btn():
    print("delete_selceted_list_delete_btn()")
